...
class WGAN_Generator(nn.Module):
    ...
    def loss(self, z, discriminator, **kwargs):
        res = self.forward(z)
        loss_fake4real = discriminator(res).mean(dim = 0)
        return - loss_fake4real
...
class WGAN_Discriminator(nn.Module):
    ...
    def loss(self, img, fake_img, **kwargs):
        real_w = self.forward(img).mean()
        fake_w = self.forward(fake_img).mean()
