...
class Cub200(Dataset):
    def __init__(self, root, split, img_size, center_crop_scale, label_to_onehot = False):
        ...
        if split not in ['train', 'test', 'whole']:
            raise AttributeError('split should be either "train", "test" or "whole".')
        self.root = root
        ...
        self.split = 0 if split == "train" else (1 if split == "test" else -1)
